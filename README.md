...
* `modelo_de_dados.png`: Diagrama solicitado para a Prova Final.
...
* ** AC4 - Dashboard Interativo:** https://datastudio.google.com/u/0/reporting/b29eae83-41ed-4ce8-9fc3-7c98eb28c765/page/OHerF
...
| :--- | :--- | :--- |
| **`id_beneficiario`** | `INTEGER` | **Chave Primária (PK):** Identificador numérico único e sequencial gerado dinamicamente durante o processo de normalização. Garante a integridade e a unicidade de cada registro de concessão de bolsa dentro da tabela fato. |
...
| **`CODIGO_EMEC_IES_BOLSA`** | `INTEGER` | **Identificador Numérico Estático / Chave Estrangeira (FK):** Atua como o código de identificação único da instituição de ensino no Ministério da Educação (MEC). No modelo relacional, é a FK que conecta o beneficiário à sua respectiva universidade na tabela `tb_instituicoes`. |
...
| **`CPF_BENEFICIARIO_BOLSA`** | `STRING` | **Identificador Único Mascarado:** Atributo de granularidade máxima que identifica o estudante de forma anônima, mantendo a conformidade com as regras de privacidade de dados. |
